...
def build_watch_keyboard(user_id: int, skin_name: str, threshold: float, symbol: str) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(f"Упадёт ниже {threshold:,.0f} {symbol}", callback_data=f"sw_below:{user_id}")],
        [InlineKeyboardButton(f"Вырастет выше {threshold:,.0f} {symbol}", callback_data=f"sw_above:{user_id}")],
    ])
...
async def check_prices(context: ContextTypes.DEFAULT_TYPE):
    ...
    for w in watches:
        ...
        if w["price_below"] and current_price <= w["price_below"]:
            triggered = True
            condition_text = f"упала ниже {w['price_below']:,.0f} руб."

        elif w["price_above"] and current_price >= w["price_above"]:
            triggered = True
            condition_text = f"выросла выше {w['price_above']:,.0f} руб."
...
async def list_watches(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for w in watches:
        if w["price_below"]:
            condition = f"Уведомить когда упадёт ниже <b>{w['price_below']:,.0f} руб.</b>"
        else:
            condition = f"Уведомить когда вырастет выше <b>{w['price_above']:,.0f} руб.</b>"
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- Выбор направления отслеживания ---
        ...
        if data.startswith("sw_below:"):
            ...
            condition = f"упадёт ниже {threshold:,.0f} {symbol}"
        else:
            action = upsert_watch(user_id, skin_name, price_above=threshold)
            condition = f"вырастет выше {threshold:,.0f} {symbol}"
